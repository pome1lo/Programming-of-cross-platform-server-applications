...
module.exports = async (req, res, db) => {
    ...
    if (endpoints.includes(endpoint)) {
        try {
            ...
            if (result.recordset.length !== 0) write202(res, result.recordset);
            else write404(res, "404");
        } catch (e) { console.log(e); write400(res, e); }
        return;
    }
    ...
    if (pulpitsByFacultyRegex.test(endpoint)) {
        ...
        try {
            ...
            if (response.recordset.length !== 0) write202(res, response.recordset);
            else write404(res, "404");
        } catch (err) { write400(res, err); }
        return;
    }
